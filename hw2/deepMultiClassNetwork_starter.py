...
from twoLayerBinary_starter import sigmoid_der, tanh_der
...
def softmax_cross_entropy_loss(Z, Y=np.array([])):
    ...
    ### CODE HERE
    F = np.max(Z,axis=0)
    A = np.exp(Z - F) / np.sum(np.exp(Z - F),axis=0,keepdims=True)
    if Y.shape[0] == 0:
        loss = []
    else:
...
def initialize_multilayer_weights(net_dims):
    ...
    for l in range(numLayers-1):
        ...
        parameters["b"+str(l+1)] = np.random.randn(net_dims[l+1],1) #CODE HERE
    return parameters
...
def classify(X, parameters):
    ...
    A0,_ = multi_layer_forward(X,parameters)
    out,_,_ = softmax_cross_entropy_loss(A0)
    Ypred = np.argmax(out,axis=0)
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.2, decay_rate=0.01):
    ...
    for ii in range(num_iterations):
        ...
        Y_one_hot = np.zeros((net_dims[-1], Y.shape[1]))
        for i in range(Y.shape[1]):
            Y_one_hot[int(Y[0, i]), i] = 1
        out,cacheOfcross,cost = softmax_cross_entropy_loss(Z,Y_one_hot)

        # Backward Prop
        ## call to softmax cross entropy loss der
        ## call to multi_layer_backward to get gradients
        ## call to update the parameters
        dZ = softmax_cross_entropy_loss_der(Y_one_hot,cacheOfcross)
