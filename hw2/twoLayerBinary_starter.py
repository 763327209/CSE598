...
def tanh_der(dA, cache):
	...
	"""
    ### CODE HERE
    output, _ = tanh(cache["Z"])
    dZ = dA * (1 - np.power(output, 2))
    return dZ
...
def sigmoid_der(dA, cache):
	...
	"""
    ### CODE HERE
    out_put, _ = sigmoid(cache["Z"])
    dZ = dA * (out_put * (1 - out_put))
    return dZ
...
def cost_estimate(A2, Y):
	...
	"""
    ### CODE HERE
    # cost = np.sum(0.5 * (Y-A2)**2)/Y.shape[1]
    cost = -np.mean(np.multiply(np.log(NEAR_0 + A2), Y) + np.multiply((1 - Y), np.log(1 - A2 + NEAR_0))) / Y.shape[1]
...
def two_layer_network(X, Y, net_dims, num_iterations=2000, learning_rate=0.1):
    ...
    for ii in range(num_iterations):
        # Forward propagation
        ### CODE HERE
        A1, cache1 = layer_forward(A0, parameters["W1"], parameters["b1"], activation="tanh")
        ...
        dA_pre = -(np.divide(Y, A2 + NEAR_0) - np.divide(1 - Y, 1 - A2 + NEAR_0))
        # dA_pre = -(Y-A2)
        dA_prev2, dW2, db2 = layer_backward(
            dA_pre, cache2, W=parameters["W2"], b=parameters["b2"], activation="sigmoid"
        )
        dA_prev1, dW1, db1 = layer_backward(dA_prev2, cache1, parameters["W1"], parameters["b1"], activation="tanh")
        # update parameters
        ### CODE HERE
...
def main():
    ...
    costs, parameters = two_layer_network(
        train_data, train_label, net_dims, num_iterations=num_iterations, learning_rate=learning_rate
    )
...
def plt_cost(costs, num_iteration):
    x = range(0, num_iteration, 10)
    plt.plot(x, costs)
    plt.xlabel("iteration")
    plt.ylabel("cost")
    plt.title("The cost vs iteration")
    plt.show()
